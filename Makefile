...
build:			## Build an image from a docker-compose file. Params: {{ v=8.1 }}. Default latest PHP 8.1
	COMPOSE_PROJECT_NAME=cache-redis PHP_VERSION=$(filter-out $@,$(v)) docker-compose -f tests/docker/docker-compose.yml up -d --build

down:			## Stop and remove containers, networks
	COMPOSE_PROJECT_NAME=cache-redis docker-compose -f tests/docker/docker-compose.yml down
...
test:			## Run tests. Params: {{ v=8.1 }}. Default latest PHP 8.1
	COMPOSE_PROJECT_NAME=cache-redis PHP_VERSION=$(filter-out $@,$(v)) docker-compose -f tests/docker/docker-compose.yml build --pull cache-redis-php
	make create-cluster
	COMPOSE_PROJECT_NAME=cache-redis PHP_VERSION=$(filter-out $@,$(v)) docker-compose -f tests/docker/docker-compose.yml run cache-redis-php vendor/bin/phpunit --colors=always
	make down
